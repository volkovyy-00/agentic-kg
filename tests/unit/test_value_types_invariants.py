...
from agentic_kg.common.value_types import (
    BLANK, BOOLEAN, CONVERTED, FLOAT, INTEGER, MAJORITY_SHARE,
    NUMERIC_AFTER_CLEANING, BARE_NUMERIC, BOOLEAN_LIKE, TEXT,
    _clean_number, classify, coerce, has_fractional_part, is_blank,
)
...
ADVERSARIAL_VALUES = [
    ...
    "9007199254740992", "9007199254740993", "9007199254740993.0",
    # int64 boundaries and just past them, both signs, both spellings
    str(INT64_LIMIT - 1), str(INT64_LIMIT), str(INT64_LIMIT + 1),
    str(-INT64_LIMIT), str(-INT64_LIMIT - 1), f"{INT64_LIMIT}.0",
    # fractional, and whole numbers wearing a fractional coat
    "42.7", "42.0", "42.000", "-0.0", "0.5",
    # money, in every shape the exporters emit
    "$42", "$42.00", "-$42.00", "$-42.00", "(42.00)", "($1,234.50)",
    "$1,234,567.89", "€10", "£10", "¥10", "₹10",
    # booleans, whole vocabulary and casing
    "yes", "YES", "no", "N", "true", "False", "y", "1", "0",
    # blanks
    "", "   ", None,
    # dirt that must never become a number
    "N/A", "-", "--", "+-42", "1,2,3", "1e5", "abc", "4 2", "$", "()",
...
def test_a_suggested_type_never_trips_the_loaders_refusal_gate(column):
    ...
    unconvertible = sum(1 for value in non_blank
                        if coerce(value, suggested)[1] != CONVERTED)
...
def test_a_column_the_converter_accepts_is_never_classified_text(column):
    ...
    for declared_type in ALL_TYPES:
        convertible = sum(1 for value in non_blank
                          if coerce(value, declared_type)[1] == CONVERTED)
...
def test_a_numeric_shape_is_never_reported_for_a_column_of_text(column):
    ...
    non_blank = [value for value in column if not is_blank(value)]
    convertible = sum(1 for value in non_blank
                      if coerce(value, declared)[1] == CONVERTED)
